...
const build = async (tempPath, manifestPath, envExtension = {}) => {
  ...
  const zipStream = createWriteStream(join(tempPath, 'WolfDevTools.zip'));
...
const postProcess = async (tempPath, destinationPath) => {
  ...
  const packedSourcePath = join(tempPath, 'WolfDevTools.zip');
  const packedDestPath = join(destinationPath, 'WolfDevTools.zip');
