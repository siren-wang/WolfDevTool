...
export function getInternalReactConstants(version: string): {
  ...
  function getDisplayNameForFiber(
    ...
    const {elementType, type, tag, key } = fiber;

    if (key && fiber.memoizedProps && (fiber.memoizedProps.type || fiber.memoizedProps?.schema?.type)) return fiber.memoizedProps.type || fiber.memoizedProps?.schema?.type;
...
export function attach(
  ...
  function shouldFilterFiber(fiber: Fiber): boolean {
    const {tag, type, key, memoizedProps } = fiber;
    ...
    const isWolfComponent = () => {
      return key && memoizedProps != null && (memoizedProps.hasOwnProperty('type') || (memoizedProps.hasOwnProperty('schema')));
    }

    if (!isWolfComponent()) return true;
    ...
    const whiteList = [
      ...
      'Affix',
      'Row',
    ].map(str => str.toLowerCase().replace(/([a-z])([A-Z])/g, '$1-$2')); // 使用正则表达式将驼峰形式转换为以 '-' 分隔的形式
