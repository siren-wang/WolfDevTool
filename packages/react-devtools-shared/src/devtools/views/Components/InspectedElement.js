...
import DocumentHashMap from 'react-devtools-shared/src/shared/documentHashMap';
...
export default function InspectedElementWrapper(_: Props): React.Node {
  ...
  const lookForOfficialDocument = (type: string) => {
    if (!type) return '';
    let url = 'https://wolf-view.meituan.com/template/WolfDoc2/index#/latest/components/';
    const route = DocumentHashMap[type];
    if (route) {
      url += route;
    }
    return url;
  ...
  const elementType = inspectedElement?.props?.schema?.type || inspectedElement?.props.type;
  const elementName = inspectedElement?.props?.name;
  const isCustomComponent = elementType && elementType.startsWith('c-');

  let docBadge = null;
  if (isCustomComponent) {
    docBadge = <div>自定义组件</div>
  } else {
    const docHref = lookForOfficialDocument(elementType);

    docBadge = docHref && (
      <a
        className={styles.ComponentDocIcon}
        href={docHref}
      ...
      <div className={styles.TitleRow} data-testname="InspectedElement-Title">
        ...
        <div className={styles.SelectedComponentName}>
          <div
            ...
            title={elementType || element.displayName}>
            {elementType || element.displayName}
          </div>
          {elementName && (
            <div className={styles.Name} title={`名称 - ${elementName}`}>
              {`name: ${elementName}`}
            </div>
          )}
        </div>
