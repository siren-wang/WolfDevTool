...
function reduceTabSelectionState(
  store: Store,
  state: State,
  action: Action,
): State {
  ...
  switch (type) {
    case 'SELECT_TAB':
      return {
        ...state,
        selectedTabID: (action: ACTION_SELECT_TAB).payload
      }
    default:
      break;
  }
      ...
      (state: State, action: Action): State => {
        ...
        switch (type) {
          ...
          case 'SET_SEARCH_TEXT':
            ...
            return state;
          case 'SELECT_TAB':
            state = reduceTabSelectionState(store, state, action);
            return state;
