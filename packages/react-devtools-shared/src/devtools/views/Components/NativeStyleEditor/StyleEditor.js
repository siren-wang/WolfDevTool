...
export function NewRow({changeAttribute, changeValue, validAttributes, attributePlaceholder}: NewRowProps) {
  ...
  return (
    <Row
      key={key}
      attribute={''}
      attributePlaceholder={attributePlaceholder || 'attribute'}
...
export function Row({
  ...
  const [localValue, setLocalValue] = useState(shouldStringify ? JSON.stringify(value) : (value || ''));
  ...
  return (
    <div className={styles.Row}>
      <Field
        className={isAttributeValid ? ((localAttribute.toLowerCase().includes('wolf') || localAttribute === 'proxyId') ? styles.WolfAttribute : styles.Attribute) : styles.Invalid}
